...
if __name__ == "__main__":
    ...
    # localize times
    for column in ['tower_installation_start', 'tower_installation_end', 'nacelle_installation_start', 'nacelle_installation_end', 'blade_installation_start', 'blade_installation_end']:
        installationTimes[column] = installationTimes[column].dt.tz_localize('Europe/Berlin')
        installationTimes[column] = installationTimes[column].dt.tz_convert('UTC')
    ...
    installationTimes.insert(loc=0,
                             column='turbine_name',
                             value=[f'turbine-{i:02d}' for i in installationTimes["installation_number"].astype(int)])
    installationTimes.insert(loc=0,
                             column='turbine_id',
                             value=[i for i in installationTimes["installation_number"].astype(int)])

    # installationTimes['Blade Installation Start'].apply(lambda dt: pd.to_datetime(dt))
    installationTimes["installation_number"] = installationTimes["installation_number"].apply(
        lambda x: int(x))
    ...
    bladeInstallationTimes.insert(loc=1,
                                  column='turbine_id',
                                  value=[i for i in bladeInstallationTimes["installation_number"].astype(int)])
