...
def cleanLIDARData(data, verbose=False, timezone='Europe/Berlin'):
    ...
    try:
        ...
        print(f'*! failed to generate datetime index, skipping: {e}')
        return pd.DataFrame()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", help="name of output directory", type=str)
    parser.add_argument("--global-output", help="used to export the global data frame", type=str)
    parser.add_argument("--input-dir", help="name of input directory", type=str)
    parser.add_argument("--output-prefix", help="output file prefix", type=str, default="lidar")
    parser.add_argument("--procs", help="number of processor to use", type=int, default=8)
    parser.add_argument("--lidar-pattern", help="glob pattern to select the files containing lidar data. If not provided, genLIDARPickle defaults to *.csv", type=str, default="*.csv")
    ...
    # insert epoch time stamp
    frames.insert(loc=0,
                  column='epoch',
                  value=frames.index.astype('int64')/1e9
                 )

    if args.verbose: print("* saving csv")
    for day, data in frames.groupby(pd.Grouper(freq='D')):
        ...
        export_csv_path = path.join(args.output_dir, "{}_{}.csv".format(args.output_prefix, datestring))
    ...
    if args.global_output:
        if args.verbose: print('* saving global csv')
        export_csv_path_global = path.join(args.output_dir, args.global_output)
        try:
            frames.to_csv(export_csv_path_global)
        except Exception as e:
