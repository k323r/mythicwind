...
def resample_acceleration(acc_df,
    ...
    if verbose:
        print(
            f'* resampling data to {resample_interval}. Start time: {acc_df.index[0]}')
    ...
    for comp in components:
        data.insert(column=f'acc_{comp}',
                    # here we transform data into m/s^2
                    value=acc_df[f'acc_{comp}'].resample(
                        resample_interval).bfill()*g,
...
def filter_integrate(acc_df,
    ...
    for component in components:
        data.insert(loc=len(data.columns),
                    value=integrate.cumtrapz(
                        data[f'acc_{component}'], t, initial=0),
    ...
    for component in components:
        data.insert(loc=len(data.columns),
                    value=integrate.cumtrapz(
                        data[f'vel_{component}'], t, initial=0),
    ...
    data.insert(loc=len(data.columns),
                value=np.sqrt(np.power(data.pos_x, 2) +
                              np.power(data.pos_z, 2)),
                column='deflection'
                )
    return data
...
def resample_filter_integrate_accelerations(df, args):
    ...
    if args.integration_interval:
        ...
        if args.verbose:
            print(
                f'* using a integration interval of {args.integration_interval} minutes')

        for _, d in df.resample(args.integration_interval):
            if args.verbose:
                print(f'* start: {d.index[0]} -> end: {d.index[-1]}')
        ...
        if args.verbose:
            print(f'* integrating accelerations')
